...
async function logActivity(db, change, initiator) {
  try {
    await db.prepare(`
      INSERT INTO car_charging_activity_log (created_at, change, initiator)
      VALUES (?, ?, ?)`)
      .bind(nowZurich(), change, initiator)
      .run();
  } catch (e) {
...
async function runCarChargingAutomation(env) {
  ...
  if (einspeisungAvg > minEinspeisungWh && mode === MODE_ONLY_SOLAR) {
    ...
    await logActivity(
      db,
      `Modus: ${modeLabel(mode)} -> ${modeLabel(MODE_MINIMAL_SOLAR)}, aktuelle Einspeisung ${einspeisungAvg.toFixed(0)} Wh`,
      "energymanager & auto update"
    );
    ...
    await logActivity(
      db,
      `Modus: ${modeLabel(mode)} -> ${modeLabel(MODE_ONLY_SOLAR)}, aktuelle Einspeisung ${einspeisungAvg.toFixed(0)} Wh`,
      "energymanager & auto update"
    );
  } else {
...
async function handleCarChargerControl(request, env) {
  ...
  const oldMode = await currentMode(env.DB, device_id);
  try {
    await setChargerMode(env, device_id, Number(charging_mode), extra);
    await logActivity(
      env.DB,
      `Modus: ${modeLabel(oldMode)} -> ${modeLabel(Number(charging_mode))}`,
      "energymanager & Lademodus ändern"
    );
...
async function handleData(table, request, env) {
  const allowed = ["solarmanager_data", "kostal_data", "solarmanager_daily_stats", "ekz_energy_prices"];
  if (table === "car_charging_activity_log") {
    const { results } = await env.DB
      .prepare("SELECT * FROM car_charging_activity_log ORDER BY id DESC LIMIT 200")
      .all();
